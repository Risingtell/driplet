...
async function depositToGateway() {
  ...
  // A deposit confirms on-chain before Circle Gateway credits it as
  // "available". Poll until the available balance reflects the deposit,
  // otherwise the first payments fail verification with a 0 balance.
  ...
  while (Date.now() - startedAt < MAX_WAIT_MS) {
    try {
      ...
      if (updated.gateway.available > 0n) {
        console.log(
          `Gateway available balance: ${updated.gateway.formattedAvailable} (ready in ${Math.round((Date.now() - startedAt) / 1000)}s)`,
        );
        return;
      }
      console.log(
        `  waiting for Gateway to credit deposit... (${Math.round((Date.now() - startedAt) / 1000)}s)`,
      );
    } catch (e) {
      console.warn(
        `  getBalances transient error, retrying: ${(e as Error).message}`,
      );
    }
    await new Promise((r) => setTimeout(r, POLL_MS));
  }
  console.warn(
    "Gateway balance still 0 after waiting — proceeding anyway; payments may fail until it settles.",
  );
}
